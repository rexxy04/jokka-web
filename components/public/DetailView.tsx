...
// IMPORT SIDEBAR KHUSUS WISATA
import PlaceSidebar from '@/components/public/PlaceSidebar';
...
const DetailView: React.FC<DetailViewProps> = ({
  ...
  // Logic Bayar (Hanya jalan untuk Event)
  const handleBuyTicket = async () => {
    const user = auth.currentUser;
    if (!user) {
      setModalContent({ title: "Login Diperlukan", message: "Silakan login terlebih dahulu untuk membeli tiket." });
    ...
    try {
      ...
      window.snap.pay(data.token, {
        onSuccess: async function(result: any) {
          ...
          setModalContent({ title: "Pembayaran Berhasil! 🎉", message: "Tiket telah berhasil dibeli dan tersimpan di menu Profil Anda." });
        ...
        onPending: async function(result: any) {
          ...
          setModalContent({ title: "Menunggu Pembayaran ⏳", message: "Silakan selesaikan pembayaran Anda. Status tiket dapat dicek di Profil." });
          setOnModalCloseAction(() => () => router.push('/profile'));
          setShowModal(true);
        },
  ...
  const handleCloseModal = () => {
      setShowModal(false);
      if (onModalCloseAction) { onModalCloseAction(); setOnModalCloseAction(null); }
  };
    ...
    <article className="bg-white min-h-screen pb-20">
      ...
      <div className="relative h-[50vh] md:h-[60vh] w-full group">
        ...
        <div className="absolute top-24 left-4 md:left-8 z-10">
           <Link href="/" className="flex items-center gap-2 text-white/80 hover:text-white transition bg-black/20 backdrop-blur-sm px-4 py-2 rounded-full">← Kembali</Link>
        </div>
      ...
      <div className="container mx-auto px-4 py-12 md:flex gap-12">
        ...
        {/* SIDEBAR: LOGIC PEMISAHAN */}
        <div className="md:w-1/3 mt-8 md:mt-0">
          
          {type === 'place' ? (
            // ===============================================
            // TAMPILAN 1: WISATA (Pakai Component PlaceSidebar)
            // ===============================================
            <PlaceSidebar 
              price={price}
              location={location}
            ...
            // ===============================================
            // TAMPILAN 2: EVENT (INLINE BOOKING + FIX MAPS URL)
            // ===============================================
            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 sticky top-24">
              ...
              {tickets.length > 0 ? (
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3 rounded-lg border cursor-pointer transition-all ${
                          ...
                          : selectedTicketIdx === idx ? 'bg-blue-50 border-blue-500 ring-1 ring-blue-500' 
                ...
                <div className="p-4 bg-yellow-50 text-yellow-700 text-sm rounded-lg mb-4">Tidak ada info tiket tersedia.</div>
              )}
              ...
              {selectedTicketIdx !== -1 && (
                <div className="flex items-center justify-between mb-6 bg-white p-3 rounded-lg border border-gray-200">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-8 h-8 rounded-full bg-gray-100 text-gray-600">-</button>
                    <span className="font-bold w-4 text-center">{qty}</span>
                    <button onClick={() => {
                        const t = tickets[selectedTicketIdx];
                        const max = t.stock - (t.sold || 0);
                        setQty(Math.min(max, qty + 1));
                    }} className="w-8 h-8 rounded-full bg-gray-100 text-gray-600">+</button>
                  </div>
                </div>
              )}
              ...
              {selectedTicketIdx !== -1 && (
                <div className="flex justify-between items-center mb-4 pt-4 border-t border-gray-200">
                  ...
                  <span className="text-xl font-bold text-gray-900">{formatRupiah(tickets[selectedTicketIdx].price * qty)}</span>
                </div>
              )}
              ...
              {/* PETA UNTUK EVENT (INLINE - FIX URL FINAL) */}
              <div className="mt-6 pt-6 border-t border-gray-200">
                 <h4 className="text-sm font-bold text-gray-700 mb-3">Lokasi Peta</h4>
                 {(() => {
                    let mapSrc = "";
                    if (lat && lng) {
                        // FIX: Pakai maps.google.com langsung (BUKAN googleusercontent)
                    ...
                    if (mapSrc) return (
                       <div className="card-glass rounded-xl overflow-hidden shadow-sm border border-gray-200 bg-white">
                          <div className="relative w-full h-[250px] bg-gray-100 flex items-center justify-center">
                            <iframe className="w-full h-full border-0" loading="lazy" allowFullScreen src={mapSrc} title="Lokasi Peta"></iframe>
                          </div>
                          <a href={mapSrc.replace("&output=embed", "")} target="_blank" rel="noopener noreferrer" className="block text-center py-2 text-xs font-bold text-blue-600 hover:bg-blue-50 transition">📍 Buka di Google Maps</a>
                       </div>
                    );
                    return <div className="p-4 bg-gray-100 rounded-xl border border-dashed text-center text-xs text-gray-500">🚫 Lokasi peta belum diatur.</div>;
                 })()}
              </div>
            </div>
          )}
      ...
      <StatusModal isOpen={showModal} onClose={handleCloseModal} title={modalContent.title} message={modalContent.message} />
    </article>
  );
};
