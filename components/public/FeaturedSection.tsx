  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Query: Ambil data, prioritaskan yang isFeatured
        // (Note: Firestore sorting by boolean kadang tricky tanpa index, jadi kita sort di client)
        ...
        // Sorting: Yang isFeatured: true naik ke atas (Index 0)
    ...
    return (
      <section className="py-16 bg-white h-[500px] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">Sedang memuat destinasi...</p>
        </div>
      </section>
    );
  }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8 flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-500">Destinasi hits minggu ini.</p>
          </div>
          <div className="hidden md:block">
            <Button href="/destinasi" variant="ghost">Lihat Semua &rarr;</Button>
          </div>
        </div>
        ...
        <div className="hidden md:grid grid-cols-4 grid-rows-2 gap-4 h-[500px]">
          {destinations.slice(0, 5).map((item) => (
            <Link 
              key={item.id}
              href={`/destinasi/${item.id}`} // <--- INI KUNCINYA (Link ke Detail)
              className={`relative group overflow-hidden rounded-2xl cursor-pointer shadow-sm hover:shadow-lg transition-all ${
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 p-5 w-full">
                ...
                <div className="flex justify-between items-end">
                  <h3 className={`font-bold text-white leading-tight ${item.isFeatured ? 'text-2xl' : 'text-lg'} group-hover:text-blue-300 transition`}>
                    {item.name}
                  </h3>
        ...
        <div className="md:hidden flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scrollbar-hide">
          {destinations.map((item) => (
            <Link 
              key={item.id} 
              href={`/destinasi/${item.id}`} // <--- Link Mobile
              className="snap-center shrink-0 w-[80vw] h-[300px] relative rounded-2xl overflow-hidden shadow-md"
            >
               <img
                src={item.image}
                alt={item.name}
        ...
        <div className="mt-6 text-center md:hidden">
          <Button href="/destinasi" variant="outline" className="w-full">Lihat Semua Destinasi</Button>
        </div>
